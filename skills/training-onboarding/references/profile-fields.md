...
- `energy.target_kcal` (optional integer): working daily target after `godkänn`. Never BMR, TDEE, macros, MET, or a protein target on the profile (`target_protein` does not exist). Meal `kcal` / `protein_g` live on `food_logged`, not here. Does not auto-update when weight changes. Follow-up may replace it after a new `godkänn`
