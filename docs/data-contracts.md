...
- Do not present remaining kcal vs `target_kcal` on the log echo. Incomplete sums vs the target belong to follow-up (or when they asked), with gaps unknown rather than 0
...
`nutrition.energy.target_kcal` is a working number. It may be replaced after `godkänn` when follow-up against training logs, optional food logs, weight, hunger, and recovery supports a change. Never auto-rewrite from a formula or a single log. How it is spoken follows `nutrition.goal` (floor / riktmärke / modest deficit). Gaps in food logs are unknown intake, not a confirmed miss vs the target.
