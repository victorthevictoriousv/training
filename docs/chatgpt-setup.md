...
Expected: `kcal` + `kcal_source = user`. No invented `protein_g`.

4c. Stated protein only (`45 g protein till middag` plus a named meal).

Expected: `protein_g` + `protein_source = user`. No invented `kcal`.

4d. Gym log (`bänk 80x5`) and vanearkivet fetch (`mina recept` / `hämta …`).
